...
Mini App открывается из `/start`, меню чата и настроенного через BotFather Main Mini App. Пользователи отправляют новые MP3 на модерацию и отслеживают свои заявки, а администраторы проверяют очередь, редактируют и обрезают аудио, затем одобряют или отклоняют заявки.
