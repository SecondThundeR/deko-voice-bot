...
export type BasicUsersStats = {
    allUsedUsers: number;
    allAnalyticsDisabledUsers: number;
    allMAUUsers: number;
...
export type FullUsersStats = Omit<SelectUser, "analyticsEnabled" | "userId">;
...
const basicStatsColumns = {
    ...
    allAnalyticsDisabledUsers: sql<number>`cast(count(*) filter (where ${usersTable.analyticsEnabled} = false) as int)`,
    allMAUUsers: sql<number>`cast(count(*) filter (where ${usersTable.analyticsEnabled} = true and ${usersTable.lastUsedAt} > ${oneMonthAgoMs}) as int)`,
    allInactiveUsers: sql<number>`cast(count(*) filter (where ${usersTable.analyticsEnabled} = true and (${usersTable.lastUsedAt} is null or ${usersTable.lastUsedAt} < ${oneMonthAgoMs})) as int)`,
...
const mostUsedUsersStats = sql<FullUsersStats[]>`(
    ...
    from (
        ...
        from ${usersTable}
        where ${usersTable.analyticsEnabled} = true and ${usersTable.usesAmount} > 0
