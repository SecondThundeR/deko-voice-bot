...
type CancelPreparedImport = (operationId: string) => Promise<void>;

async function cleanupPreparedImport(
    session: ImportSession,
    cancelPreparedImport: CancelPreparedImport,
) {
    if (session.stage !== "awaiting-confirmation") {
        return;
    }

    await cancelPreparedImport(session.operationId).catch(() => {});
}
...
export class ImportSessionStore {
    ...
    readonly #cancelPreparedImport: CancelPreparedImport;

    constructor(cancelPreparedImport: CancelPreparedImport = async () => {}) {
        this.#cancelPreparedImport = cancelPreparedImport;
    }
    ...
    async cancel(userId: number, chatId: number) {
        ...
        await cleanupPreparedImport(session, this.#cancelPreparedImport);
        return true;
    }
    ...
    async #delete(key: SessionKey, expectedSession: ImportSession) {
        ...
        this.#removeFromMemory(key);
        await cleanupPreparedImport(
            expectedSession,
            this.#cancelPreparedImport,
        );
    }
}
