...
import {
    beginGlobalDatabaseMaintenance,
    endGlobalDatabaseMaintenance,
    lockGlobalDatabaseTraffic,
...
feature.callbackQuery(
    ...
    async (ctx) => {
        ...
        let globalMaintenanceClient:
            | Awaited<ReturnType<typeof beginGlobalDatabaseMaintenance>>
            | undefined;
        let restored = false;
        ...
        try {
            ...
            globalMaintenanceClient = await beginGlobalDatabaseMaintenance();
            await lockGlobalDatabaseTraffic(globalMaintenanceClient);
            ...
            if (globalMaintenanceClient) {
                await endGlobalDatabaseMaintenance(
                    globalMaintenanceClient,
                    restored,
                ).catch((maintenanceError) => {
                    ctx.logger.error({
                        msg: "Failed to end global database maintenance",
                        ...getSafeErrorInfo(maintenanceError),
                    });
                });
            }
            if (maintenanceStarted) {
