...
const baseConfigSchema = v.object({
    ...
    webAppUrl: v.optional(v.pipe(v.string(), v.url())),
    moderationChatId: v.optional(
        v.pipe(v.string(), v.transform(Number), v.safeInteger()),
    ),
    redisUrl: v.optional(
        v.pipe(
            v.string(),
...
const configSchema = v.pipe(
    ...
    v.forward(
        v.check(
            (input) => input.nodeEnv !== "production" || !!input.webAppUrl,
            "WEB_APP_URL is required in production",
        ),
        ["webAppUrl"],
    ),
    v.forward(
        v.check(
            ...
            "VOICE_MODERATION_CHAT_ID is required in production",
        ),
        ["moderationChatId"],
    ),
