...
import { OPERATIONS_ROUTES } from "@deko-voice-bot/operations-contract";
...
export function createOperationsServer(
    ...
    server.get(OPERATIONS_ROUTES.health, (c) =>
        c.json({
    ...
    server.post(OPERATIONS_ROUTES.voiceConvert, async (c) => {
    ...
    server.post(OPERATIONS_ROUTES.exportDatabase, async () => {
        const paths = await createBackupTempPaths("operations-export");
        try {
    ...
    server.post(
        ...
        async (c) => {
            ...
            const paths = await createBackupTempPaths("operations-pre-import");
            try {
                const sha256 = await withBackupAdvisoryLock(
                    config.databaseUrl,
                    () =>
    ...
    server.onError((error, c) => {
        ...
        return c.json({ error: "Operations request failed" }, 500);
    });
