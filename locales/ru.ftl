...
stats-message =
    Всего пользователей бота: { NUMBER($allUsedUsers, useGrouping: 0) } (без аналитики: { NUMBER($allAnalyticsDisabledUsers, useGrouping: 0) })
...
my-data-favorites-count = - Реплик в избранном: { NUMBER($favoritesCount, useGrouping: 0) }

my-data-analytics-disabled =
    Сбор вашей личной статистики отключён.
    - Сохранённый Telegram ID: <code>{ NUMBER($userId, useGrouping: 0) }</code>
    - Реплик в избранном: { NUMBER($favoritesCount, useGrouping: 0) }

    Имя, username, количество использований и время последнего использования удалены. Избранное продолжает работать. Платёжные записи сохраняются отдельно и описаны в политике конфиденциальности: /privacy
...
opt-out-success-header =
    Сбор вашей личной статистики отключён. Ниже приведены статистические данные, которые были удалены:

opt-out-no-previous-statistics = - Личная статистика ещё не была накоплена

opt-out-success-footer =
    Telegram ID и избранное сохранены, чтобы помнить ваш выбор и продолжать работу избранного. Платёжные записи также не удаляются. Подробнее: /privacy
...
voices-add-button = Добавить реплику
