...
maintenance =
    ...
    .description-chat = На данный момент, бот находится в режиме технических работ и любые команды пока что недоступны. Это может продлится от нескольких минут до часов
...
favorites =
    .header = Выберите реплики, которые вы хотите сделать избранными. Они будут появляться в самом начале списка
    .prev = <
    .close = Закрыть
    .next = >
...
menu =
    ...
    .failedToUpdate = Telegram не позволяет обновить это сообщение после 48 часов. Удалите сообщение вручную
    .failedToDelete = Не удалось удалить это меню. Возможно, прошло более 48 часов, после чего, Telegram не даёт удалять мне сообщения. Удалите сообщение вручную
