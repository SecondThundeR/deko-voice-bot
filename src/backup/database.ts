...
export async function normalizeRestoredDatabase(databaseUrl: string) {
    ...
    try {
        await client.begin(async (transaction) => {
            ...
            const userColumns = await transaction<{ columnName: string }[]>`
                select column_name as "columnName"
                from information_schema.columns
                where table_schema = 'public' and table_name = 'users'
            `;
            const columnNames = new Set(
                userColumns.map(({ columnName }) => columnName),
            );
            if (
                columnNames.has("is_ignored") &&
                !columnNames.has("analytics_enabled")
            ) {
                await transaction`
                    alter table users rename column is_ignored to analytics_enabled
                `;
                await transaction`
                    alter table users alter column analytics_enabled set default true
                `;
                await transaction`
                    update users set analytics_enabled = not analytics_enabled
                `;
                await transaction`drop index if exists users_active_last_used_at_idx`;
                await transaction`
                    create index users_active_last_used_at_idx
                    on users (last_used_at desc nulls last)
                    where analytics_enabled = true and uses_amount > 0
                `;
            }
